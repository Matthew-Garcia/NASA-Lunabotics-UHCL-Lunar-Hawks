...
def wait_for(name,predicate,seconds=25):
    ...
    while time.monotonic()<until:
        step()
        if predicate():checks.append(name);return
...
try:
    ...
    x=data['odom'].pose.pose.position.x;t=Twist();t.linear.x=.05
    until=time.monotonic()+3
    while time.monotonic()<until:drive.publish(t);step()
    drive.publish(Twist());assert abs(data['odom'].pose.pose.position.x-x)>.02
