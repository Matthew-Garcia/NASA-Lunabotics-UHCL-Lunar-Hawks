...
ramp=link('ramp','gate',1)
c=sub(ramp,'collision');sub(c,'origin',xyz='0 0 .12');sub(sub(c,'geometry'),'box',size='.008 .55 .24')
joint('ramp_joint','bucket','ramp',typ='revolute',limits=(-1.6,0))
...
joint('conveyor_mount','base_link','conveyor','.90 0 -.19','0 -.488692 0')
...
for side,y in [('left',.26),('right',-.26)]:
 link('actuator_'+side,box='.04 .04 .36',m=.7)
 joint('actuator_'+side+'_mount','base_link','actuator_'+side,f'-.2 {y} .16','0 .5 0')
for side in ['left','right']:
 link('actuator_rod_'+side,box='.016 .016 .25',m=.2)
...
for n in ['wheel_fl_joint','wheel_fr_joint','wheel_rl_joint','wheel_rr_joint','bucket_joint','ramp_joint','conveyor_drive_joint']+['actuator_rod_left_joint','actuator_rod_right_joint']+[f'scoop_{i}_joint' for i in range(7)]:txt(js,'joint_name',n)
