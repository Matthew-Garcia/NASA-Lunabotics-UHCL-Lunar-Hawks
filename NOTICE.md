# Provenance and scope

Migrated from https://github.com/Matthew-Garcia/uhcl-lunar-hawks-lunabotics at commit ae699a2465fea33d9bb36226493400fb5ad2afa8 on 2026-09-08. The original repository is unchanged.

| Original path | New path | Changes |
| --- | --- | --- |
| Laptop/firmware/twist_subscriber/ | firmware/micro_ros/esp32_twist_subscriber/ | None |
| Nvidia Jetson Orin NX/lunabotics_teleop/ | ros2_ws/src/lunabotics_teleop/ | Portable launch, preview topic, metadata/dependencies; cache excluded |
| CAD/*.stl | cad/ | None |
| assets/ | media/ | None |
| docs/software PDF | docs/legacy/ | None; historical document may contain stale instructions |

The original MIT license is preserved. Third-party assets retain their applicable rights. Team logos, vendor imagery, adapted CAD, and future manuscripts are not automatically relicensed by their presence here. Confirm authorship and redistribution terms before broader reuse. No NASA or university endorsement is implied.
